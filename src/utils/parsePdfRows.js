// Pure logic for turning pdfjs text items into the same tab-separated text the
// paste flow produces: `name\tdate\taccount\taction\tamount` per line. No pdfjs / no
// DOM here, so it runs on the server (the parsing server action) and is unit-tested
// directly. The downstream pipeline (parseTextToRows / parseAndPrepareRows) then
