...
import { MoneyFilters } from "./MoneyFilters";
...
export default async function MoneyPage({ searchParams }) {
    ...
    const { year = defaultYear, month = defaultMonth, account, category } = await searchParams;
    ...
    const filteredExpenses = selectedCategories.length > 0
        ? existingExpenses.filter((expense) => selectedCategories.includes(expense.category))
        : existingExpenses;

    const data = getData(filteredExpenses, year, month);

    const currentDate = new Date(2000 + Number(year), Number(month) - 1);
    const nextDate = addMonths(currentDate, 1);
    const prevDate = subMonths(currentDate, 1);

    const buildMonthHref = (target) => {
        const params = new URLSearchParams({
            year: format(target, "yy"),
